...
print(_path)
dumped_classifier = "/mnt/data/group3/tin/tink/dumped_classifier.pkl"
...
def tag_named_entities(text):
    ...
    for i, sentence in enumerate(test_sents):
        result.append([(word, tag)
                       for word, tag in zip(sentence.split(), y_pred[i])])
    return text, result
...
def tag_document(text, level=-1):
    # print(text, file=sys.stdout)
    text, predictedNerText = tag_named_entities(text)
    predictedNerText = get_output_level(predictedNerText, level=level)
    return text, predictedNerText


# News article extraction

def tag_news_article(url, level=-1):
    text = extract_news_article(url)
    # print(text, file=sys.stdout)
    return tag_document(text, level)
